...
import lattice_calculator_procedural2 as lattice_calculator
...
#            %---------------------------------------------------------------------------------------------
    ...
    def ResMatS(self,H,K,L,W,EXP):
        ...
        mylattice=self.lattice_calculator
        Q=2*pi*lattice_calculator.modvec(H,K,L,'latticestar',mylattice)
        uq=N.zeros((3,self.lattice_calculator.npts),'d')

        uq[0,:]=H/Q;  #% Unit vector along Q
        uq[1,:]=K/Q;
        uq[2,:]=L/Q;
        xq=2*pi*lattice_calculator.scalar(x[0,:],x[1,:],x[2,:],uq[0,:],uq[1,:],uq[2,:],'latticestar',mylattice);
        yq=2*pi*lattice_calculator.scalar(y[0,:],y[1,:],y[2,:],uq[0,:],uq[1,:],uq[2,:],'latticestar',mylattice);
...
#        %Sample shape matrix in coordinate system defined by scattering vector
    ...
    def ResPrint(self, H,K,L,W, EXP,mylattice):
        import resulttable

        #Q=lattice_calculator.modvec(H,K,L,'latticestar')
        Q=2*pi*lattice_calculator.modvec(H,K,L,'latticestar',mylattice)
        cor = self.calc_correction(H,K,L,W,EXP)
        widths = self.CalcWidths(H,K,L,W,EXP)
        R0,RM=self.ResMat(Q,W,EXP)
        R0,RMS=self.ResMatS(H,K,L,W,EXP)

        data = ["h",H,"k",K,"l",L,"&omega;",W,"Q (&Aring)",Q,
                "&theta; correction", cor["th_correction"],
                "2&theta; correction", cor["tth_correction"],
                "&theta; width cross-section (degrees)", widths['thwidth_sec'],
                "&theta; width projection (degrees)", widths['thwidth_proj'],
                "2&theta; width cross-section (degrees)", widths['tthwidth_sec'],
                "2&theta; width projection (degrees)", widths['tthwidth_proj'],
                "&omega; width cross-section (meV)", widths['WBWidth'],
                "QX Lab width cross-section (&Aring)", widths['XBWidth'],
                "QY Lab width cross-section (&Aring)", widths['YBWidth'],
                "&omega; width projection (meV)", widths['WWidth'],
                "QX Lab width projection (&Aring)", widths['XWidth'],
                "QY Lab width projection (&Aring)", widths['YWidth'],
                "QZ width projection (&Aring)", widths['ZWidth'],
                "R<sub>0</sub>", R0[0],
                "R<sub>CN</sub>", RM.T,
                "R<sub>lab</sub>", RMS.T,
                ]
        resulttable.render(data)
    ...
    def ResPlot(self,H,K,L,W,EXP):
        ...
        GridPoints=101
        logging.info('Resplot 1')      
        [R0,RMS]=self.ResMatS(H,K,L,W,EXP)
        #[xvec,yvec,zvec,sample,rsample]=self.StandardSystem(EXP);
        mylattice=self.lattice_calculator
        lattice_calculator.StandardSystem(self.lattice_calculator.orient1,self.lattice_calculator.orient2,mylattice)
        #print 'shape ',self.lattice_calculator.x.shape
        qx=2*pi*lattice_calculator.scalar(self.lattice_calculator.x[0,:],self.lattice_calculator.x[1,:],self.lattice_calculator.x[2,:],H,K,L,'latticestar',mylattice)
        qy=2*pi*lattice_calculator.scalar(self.lattice_calculator.y[0,:],self.lattice_calculator.y[1,:],self.lattice_calculator.y[2,:],H,K,L,'latticestar',mylattice)
        qw=W;

        #========================================================================================================
        #find reciprocal-space directions of X and Y axes

        o1=self.lattice_calculator.orient1.T #EXP['orient1']
        o2=self.lattice_calculator.orient2.T #EXP['orient2']
        pr=lattice_calculator.scalar(o2[0,:],o2[1,:],o2[2,:],self.lattice_calculator.y[0,:],self.lattice_calculator.y[1,:],self.lattice_calculator.y[2,:],'latticestar',mylattice)
        ...
        logging.info('Resplot 2')     

        rsample='latticestar'
        oxmax=XMax/lattice_calculator.modvec(o1[0],o1[1],o1[2],rsample,mylattice)
        oxmin=XMin/lattice_calculator.modvec(o1[0],o1[1],o1[2],rsample,mylattice)
        oymax=YMax/lattice_calculator.modvec(o2[0],o2[1],o2[2],rsample,mylattice)
        oymin=YMin/lattice_calculator.modvec(o2[0],o2[1],o2[2],rsample,mylattice)
        #print 'a1 ',a1
        #print 'b1 ',b1
        #print 'theta ',theta
        ...
        ylabel=r'Q$_y$' +'(units of [%g %g %g]'%(o2[0,center],
                                                   o2[1,center], o2[2,center]);
        ...
        if 1:
            ...
            ax3.yaxis.set_major_formatter(pylab.NullFormatter())
            ax3.yaxis.set_major_locator(pylab.NullLocator())
            xlabel=r'Q$_x$' +'(units of [%g %g %g]'%(o1[0,center],
                                                        o1[1,center], o1[2,center]);
        ...
        #print 'theta_sec ',theta_sec
        rsample='latticestar'
        oxmax=YMax/lattice_calculator.modvec(o2[0],o2[1],o2[2],rsample,mylattice)
        oxmin=YMin/lattice_calculator.modvec(o2[0],o2[1],o2[2],rsample,mylattice)
        oymax=WMax
        oymin=WMin
        #print 'a1 ',a1
        #print 'b1 ',b1
        ...
        if 1:
            ...
            ax3.yaxis.set_major_formatter(pylab.NullFormatter())
            ax3.yaxis.set_major_locator(pylab.NullLocator())
            xlabel=r'Q$_y$' +'(units of [%g %g %g]'%(o2[0,center],
                                                        o2[1,center], o2[2,center])
            ax3.set_xlabel(xlabel)
            #ax3.set_zorder(2)
        ...
        logging.info('About to write the file')
        logging.info(os.getcwd())
        pylab.savefig("resout.png")
        logging.info('Wrote the file')
    ...
    def calc_correction(self,H,K,L,W,EXP,qscan=None):
        ...
        #Q=2*pi*lattice_calculator.modvec(H,K,L,'latticestar',mylattice)
        mylattice=self.lattice_calculator
        #sys.stderr.write('correcting')
        #sys.stderr.write(mylattice.a)
        #sys.stderr.write('a %5.2f b %5.2f c %5.2f\n'%(mylattice.a[0],mylattice.b[0],mylattice.c[0]))  
        #sys.stderr.write('alpha %5.2f beta %5.2f gamma %5.2f\n'%(mylattice.alpha[0],mylattice.beta[0],mylattice.gamma[0]))  
        Q=2*pi*lattice_calculator.modvec(H,K,L,'latticestar',self.lattice_calculator)
        #sys.stderr.write('H %5.2f K %5.2f L %5.2f Q %5.2f \n'%(H,K,L,Q))
        R0,RM=self.ResMat(Q,W,EXP)

        #calculate two theta for correction.  
        CONVERT2=2.072
        #mono=[EXP['mono']]
        npts=len(EXP)
        ...
        instrument=lattice_calculator.Instrument()
        for ind in range(npts):
        ...
        #x,y,z=self.lattice_calculator.StandardSystem()
        #qx,qy,qz,Q=lattice_calculator.R2S(H,K,L,mylattice)
        ...
        ei=efixed+W
        ef=efixed+0*W
        #sys.stderr.write('Ei %5.2f Ef %5.2f \n'%(ei,ef,))
        change=N.where(infin>0)
        if N.size(change)!=0:
            ef[change]=efixed[change]-W[change]
            ei[change]=efixed[change]

        ki = N.sqrt(ei/CONVERT2)
        kf = N.sqrt(ef/CONVERT2) 
        #sys.stderr.write('ki %5.2f kf %5.2f \n'%(ki,kf,))         
        S2=N.arccos((ki**2+kf**2-Q**2)/(2*ki*kf))
        #sys.stderr.write('\n top %5.2f bottom %5.2f\n'%(ki**2+kf**2-Q**2,2*ki*kf))
        th_correction=[]
        tth_correction=[]
        if qscan != None:
            qx=[]
            ...
            for i in range(len(qscan)):
                ...
                qscan_mod=lattice_calculator.modvec(qh,qk,ql,'latticestar')
        ...
        for i in range(RM.shape[2]):
            ...
            #print 'Q', Q[i]
            tth_correction.append(N.sqrt(N.linalg.det(RM[:,:,i]))* N.tan(0.5*S2[0,i])/N.sqrt(Mxx)/Q[i])
            if qscan!=None:
    ...
    def CalcWidths(self,H,K,L,W,EXP):
        ...
        Q=2*pi*lattice_calculator.modvec(H,K,L,'latticestar',self.lattice_calculator)
        #sys.stderr.write('H %5.2f K %5.2f L %5.2f\n'%(H,K,L))
        #sys.stderr.write('Q %5.2f'%(Q,))
        #print 'Q ',Q
...
#        print 'yshape ',self.lattice_calculator.y.shape
        ...
        XWidth=2*self.fproject(RMS,0) #fproject returns HWHM, so XWidth is FWHM
        ...
        XWidthc=1*self.fproject(RMc,0)  #these are HWHM values, RMc uses ResMat instead of ResMatS
        ...
        tthwidth_sec=2*2*XBWidthc*N.tan(0.5*S2)/Q #first 2 converts to FWHM from HWHM, and the second 2 is a result of taking dQ w.r.t. to 2theta and not just theta; N.tan(0.5*S2)/Q converts from recip. to real space
        tthwidth_proj=2*2*XWidthc*N.tan(0.5*S2)/Q
        thwidth_sec=(2*YBWidthc)/Q   #first 2 converts to FWHM from HWHM; /Q converts from recip. to real space
        thwidth_proj=(2*YWidthc)/Q
...
def parse_file(infile):
    ...
    while True:
        ...
        if i==0:
            ...
            EXP['hcol']=N.array([float(toks[0]), float(toks[1]),
                                 float(toks[2]), float(toks[3])],'d')
        elif i==2:
            EXP['vcol']=N.array([float(toks[0]), float(toks[1]),
                                 float(toks[2]), float(toks[3])],'d')
        elif i==3:
    ...
    npts = max(len(v) for v in (H,K,L,W))
    if N.any([len(v) != 1 and len(v) != npts for v in (H,K,L,W)]):
        raise ValueError("H,K,L,W lengths do not match")
    H,K,L,W = [N.ones(npts)*v for v in (H,K,L,W)]
    return dict(a=a,b=b,c=c,alpha=alpha,beta=beta,gamma=gamma,
                H=H,K=K,L=L,W=W,
                orient1=orient1,orient2=orient2,
                EXP=EXP)

def process(a,b,c,alpha,beta,gamma,orient1,orient2,EXP,
            H,K,L,W,plot=False,table=True):

    #orientation=lattice_calculator.Orientation(orient1,orient2)
    #mylattice=lattice_calculator.Lattice(a=a,b=b,c=c,
    #                                     alpha=alpha,beta=beta,gamma=gamma,
    #                                     orientation=orientation)
    setup=[EXP]
    #myrescal=rescalculator(mylattice)
    #newinput=lattice_calculator.CleanArgs(a=a,b=b,c=c,
    #                                      alpha=alpha,beta=beta,gamma=gamma,
    #                                      orient1=orient1,orient2=orient2,
    #                                      H=H,K=K,L=L,W=W,setup=setup)
    #neworientation=lattice_calculator.Orientation(newinput['orient1'],
    #                                              newinput['orient2'])
    #mylattice=lattice_calculator.Lattice(
    #    a=newinput['a'],b=newinput['b'],c=newinput['c'],
    #    alpha=newinput['alpha'],beta=newinput['beta'],gamma=newinput['gamma'],
    #    orientation=neworientation)
    #myrescal.__init__(mylattice)
    orientation=lattice_calculator.Orientation(orient1,orient2)
    mylattice=lattice_calculator.Lattice(a=a,b=b,c=c,alpha=alpha,beta=beta,gamma=gamma, orient1=orient1,orient2=orient2)
    myrescal=rescalculator(mylattice)
    ...
    mylattice=lattice_calculator.Lattice(a=newinput['a'],b=newinput['b'],c=newinput['c'],alpha=newinput['alpha'],\
                                         beta=newinput['beta'],gamma=newinput['gamma'],orient1=newinput['orient1'],orient2=newinput['orient2'])
    myrescal.__init__(mylattice)

    if plot: 
        myrescal.ResPlot(H, K, L, W, setup)
    if table:
        myrescal.ResPrint(H, K, L, W, setup,mylattice)
    #if plot: 
    #    myrescal.ResPlot(H, K, L, W, setup)
...
def example1():
    ...
    return dict(a=a,b=b,c=c,alpha=alpha,beta=beta,gamma=gamma,
                H=H,K=K,L=L,W=W,orient1=orient1,orient2=orient2,
                EXP=EXP)
...
def main():
    # Process --table and --plot arguments
    ...
    log_dir=r"/tmp"
    log_fname = os.path.join(log_dir, 'example.log')    
    ...
    while len(sys.argv) > 1 and sys.argv[1].startswith('-'):
        if sys.argv[1] == '--table': 
            ...
            raise ValueError("unknown option "+sys.argv[1])
        del sys.argv[1]
    if not table and not plot:
        table = plot = True
    #opts = parse_file('rescalc_in.txt')
    #opts = example1()
    #opts = example2()
    try:
        opts = parse_file(sys.argv[1])
        process(plot=plot, table=table, **opts)
    except:
        import traceback
        print("<pre>")
...
if __name__=="__main__":
    #opts=example1()
    #process(plot=False, table=True, **opts)
    main()
